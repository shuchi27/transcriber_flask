...
def transcribe_with_whisper_audio(video_url, output_path="downloaded_audio.mp3"):
    ...
    try:
        ...
        return json.dumps({
            "error": "Audio extraction failed. This video may be private or unsupported by yt-dlp.",
...
def download_subtitles(url):
    ...
    try:
        ...
        if "OAuth" in stderr or "HTTP Error 400" in stderr:
            return json.dumps({"error": "This Vimeo video is restricted or private. Cannot fetch transcript."})
        return json.dumps({"error": f"yt-dlp failed: {stderr}"})
...
if __name__ == "__main__":
    ...
    try:
        ...
        if "error" in parsed:
            ...
            try:
                parsed_whisper = json.loads(whisper_transcript)
                print(json.dumps(parsed_whisper))
            except json.JSONDecodeError:
                print(json.dumps({"error": "Whisper transcription failed"}))
        else:
            print(transcript)

    except json.JSONDecodeError:
    print(json.dumps({"error": "Transcript failed"}))
